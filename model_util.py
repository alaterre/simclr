...
def add_weight_decay(adjust_per_optimizer=True):
    ...
    tf.losses.add_loss(
        FLAGS.weight_decay * tf.add_n(l2_losses), tf.GraphKeys.REGULARIZATION_LOSSES
    )
...
def learning_rate_schedule(base_learning_rate, num_examples):
    ...
    warmup_steps = int(
        round(FLAGS.warmup_epochs * num_examples // FLAGS.train_batch_size)
    )
    scaled_lr = base_learning_rate * FLAGS.train_batch_size / 256.0
    learning_rate = (
        tf.to_float(global_step) / int(warmup_steps) * scaled_lr
        if warmup_steps
        else scaled_lr
    )
...
def linear_layer(
    x, is_training, num_classes, use_bias=True, use_bn=False, name="linear_layer"
):
